        ...
        max_time = 30)
{
    ...
    check_list <- check_real(param_tau, label = "Tau", 
                             min_x = 0, min_inc = TRUE, rtn = check_list)
    
    check_list <- check_real(max_time, label = "Maximum Time", 
                             min_x = 0, min_inc = TRUE, rtn = check_list)
        ...
        add_mutation_flag = TRUE)
{
    ...
    if (!is.null(coal_tree)) {
        ...
        plot(box_x, box_y, type = "n", yaxt = "n", bty = "n", 
             main = "", xlab = "", ylab = "")

        branch_col <- branch_color_palette(
            coal_tree, 
        ...
        for (node_i in 1:(2 * n - 2)) {
            ...
            segments(node_x[node_i], node_y[node_i], 
                     node_x[parent_i], node_y[node_i], 
                     col = branch_col[coal_tree_df$nb_leaf[node_i]], lwd = 2)
            
            segments(node_x[parent_i], node_y[node_i], 
                     node_x[parent_i], node_y[parent_i], 
                     col = "gray", lwd = 2)
        }
...
#==============================================================================
compute_pairwise_difference <- function(
        coal_tree_df, 
        i, 
        j)
{
        ...
        MD_Y_scale_flag = FALSE)
{
    if (!is.null(coal_tree)) {
        ...
        plot(x, y, type = "h", 
             xlim = c(0, max_time), ylim = c(0, max_y), 
             lwd = 2, col = "gray", 
             main = "", xlab = "", ylab = "")
        ...
        stat_S)
{
    ...
    e1 <- c1 / a1
    e2 <- (c2) / (a1^2 + a2)
    stat_tajima_D <- (stat_pi - stat_S / a1) / 
        sqrt(e1 * stat_S + e2 * stat_S * (stat_S - 1))
...
compute_SFS_stat <- function(ksi_i)
{
    ...
    return(list(stat_S = stat_S, stat_pi = stat_pi, stat_eta = stat_eta, 
                stat_theta_S = stat_theta_S, 
                stat_theta_pi = stat_theta_pi, 
        ...
        std_flag = FALSE)
{
    ...
    if (!is.null(coal_tree)) {
        ...
        stat_DAF <- rep(coal_tree_df$nb_leaf[-nb_node], 
                        coal_tree_df$nb_mut[-nb_node])
        i <- 1:(n - 1)
        ...
        y <- ksi_i
        branch_col <- branch_color_palette(
            coal_tree, branch_color_type = branch_color_type)
        ...
        if (std_flag) {
            plot(x, y, type = "l", 
                 xlim = c(1, max_x), ylim = c(min_y, max_y), 
                 lwd = 1, lty = 2, 
                 main = "", xlab = "", ylab = "")
        } else {
            plot(x, y, type = "h", 
        ...
        # text(max_x, 0.85 * max_y, sprintf("Pi = %.3f", 
        #                                   stat$stat_pi), pos = 2)
        
        text(max_x, 0.85 * max_y, sprintf("Eta = %d", 
                                          stat$stat_eta), pos = 2)
        
        text(max_x, 0.75 * max_y, sprintf("Theta_S = %.3f", 
                                          stat$stat_theta_S), pos = 2)
        
        text(max_x, 0.65 * max_y, sprintf("Theta_Pi = %.3f", 
                                          stat$stat_theta_pi), pos = 2)
        
        text(max_x, 0.55 * max_y, sprintf("Tajima_D = %.3f", 
                                          stat$stat_tajima_D), pos = 2)
        ...
        if (stat$stat_S == 0) {
            text(0.5 * n, 0.5 * max_y, "No polymorphism", 
                 col = "red", adj = 0.5)
        }
        ...
        max_time = 30)
{
    ...
    if (param_growth_factor > 1.0) {
        val_x <- c(0, param_tau, param_tau, max_time)
        val_y <- c(param_growth_factor, param_growth_factor, 1, 1)
        val_y <- param_theta_0 * val_y
        lines(val_x, val_y, lty = 1, col = "red", lwd = 3)
    } else if (param_growth_factor < 1.0) {
        val_x <- c(0, param_tau, param_tau, max_time)
        val_y <- c(param_growth_factor, param_growth_factor, 1, 1)
        val_y <- param_theta_0 * val_y
        lines(val_x, val_y, lty = 1, col = "blue", lwd = 3)
    } else {
        val_x <- c(0, max_time)
        val_y <- c(param_theta_0, param_theta_0)
        lines(val_x, val_y, lty = 1, col = "black", lwd = 3)
    }
        ...
        ExpTimeFlag = FALSE)
{
    ...
    if (param_checking$flag) {
        ...
        # Scale 
        height[which_pre_growth] <- param_tau + 
            (height[which_pre_growth] - param_tau) / param_growth_factor
        
        # Vector storing the index of the parent of each node
        parent <- rep(NA, nb_node)
        
        # Vector storing the number of mutations on the branch to the parent 
        #   of each node 
...
# do_plot(param_n = param_n, param_theta_0 = param_theta_0, param_growth_factor = param_growth_factor, param_tau = param_tau, max_time = max_time, MD_Y_scale_flag = MD_Y_scale_flag, DAF_Y_scale_flag = DAF_Y_scale_flag, branch_color_type = branch_color_type)
